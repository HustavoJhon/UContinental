...
First: Calculating all the prime number up till a certainn range using the standard Sieve of Eratosthenes.

Second: Using the prime numbers to reduce the given number and thus find all its prime factors.

The complexity of the solutionnn involves approx. O(n logn) in calculatin siece of eratosthenes O(log n) in calculating the prime factors of the number. So in total approx. O(n logn).
